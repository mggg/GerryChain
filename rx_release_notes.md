...
GerryChain now uses its own `random.Random` instances instead of the stdlib's global functions.
Consequences for users:

- `random.seed(N)` in user scripts NO LONGER controls GerryChain. Pass an integer seed or
  `random.Random` instance via `rng=` to `MarkovChain`, `Partition.from_random_assignment`, or
...
* def **getattr**(self, \_\_name: str) -> Any:
  => ???

* def **getitem**(self, \_\_name: str) -> Any:
  => ???
