...
macro_rules! custom_vec_iter_impl {
    ($name:ident, $iter:ident, $reversed:ident, $data:ident, $T:ty, $doc:literal) => {
        #[doc = $doc]
        #[pyclass(module = "gerrychain.rustworkx", sequence, from_py_object)]
        #[derive(Clone)]
        pub struct $name {
            pub $data: Vec<$T>,
        }
...
macro_rules! py_iter_protocol_impl {
    ($name:ident, $data:ident, $T:ty) => {
        #[pyclass(module = "gerrychain.rustworkx", skip_from_py_object)]
        pub struct $name {
            pub $data: Vec<$T>,
            iter_pos: usize,
        }
...
macro_rules! custom_hash_map_iter_impl {
    (
        ...
        #[doc = $doc]
        #[pyclass(mapping, module = "gerrychain.rustworkx", from_py_object)]
        #[derive(Clone)]
        pub struct $name {
