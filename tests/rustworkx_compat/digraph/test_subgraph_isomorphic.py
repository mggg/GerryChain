...
class TestSubgraphIsomorphic(unittest.TestCase):
    ...
    def test_non_induced_grid_subgraph_isomorphic(self):
        g_a = rustworkx.generators.directed_grid_graph(2, 2)
        g_b = rustworkx.PyDiGraph()
    ...
    def test_subgraph_vf2_mapping(self):
        ...
        mapping = rustworkx.digraph_vf2_mapping(graph, second_graph, subgraph=True)
    ...
    def test_subgraph_vf2_all_mappings(self):
        ...
        mapping = rustworkx.digraph_vf2_mapping(graph, second_graph, subgraph=True, id_order=True)
        self.assertEqual(next(mapping), {0: 0, 1: 1})
        self.assertEqual(next(mapping), {2: 1, 1: 0})

    def test_subgraph_vf2_mapping_vf2pp(self):
        graph = rustworkx.generators.directed_grid_graph(3, 3)
        second_graph = rustworkx.generators.directed_grid_graph(2, 2)
    ...
    def test_empty_digraph_subgraph_vf2_mapping(self):
        ...
        for id_order in [False, True]:
            with self.subTest(id_order=id_order):
                mapping = rustworkx.digraph_vf2_mapping(g_a, g_b, id_order=id_order, subgraph=True)
