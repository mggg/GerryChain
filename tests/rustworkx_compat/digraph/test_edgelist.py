...
class TestEdgeList(unittest.TestCase):
    ...
    def test_comment_digraph(self):
        with tempfile.NamedTemporaryFile("wt") as fd:
            ...
            fd.write("//2 3\n")
            fd.flush()
            graph = rustworkx.PyDiGraph.read_edge_list(fd.name, comment="//")
        self.assertEqual(graph.node_indexes(), [0, 1, 2])
        self.assertTrue(graph.has_edge(0, 1))
        self.assertTrue(graph.has_edge(1, 2))
        self.assertFalse(graph.has_edge(1, 0))
    ...
    def test_delim_digraph(self):
        with tempfile.NamedTemporaryFile("wt") as fd:
            ...
            fd.write("//2|3\n")
            fd.flush()
            graph = rustworkx.PyDiGraph.read_edge_list(fd.name, comment="//", deliminator="|")
        self.assertEqual(graph.node_indexes(), [0, 1, 2])
        self.assertTrue(graph.has_edge(0, 1))
        self.assertTrue(graph.has_edge(1, 2))
        self.assertFalse(graph.has_edge(1, 0))
    ...
    def test_labels_digraph(self):
        with tempfile.NamedTemporaryFile("wt") as fd:
            fd.write("a|b|0// test a comment\n")
            fd.write("b|c|1\n")
            fd.write("//c|d\n")
            fd.flush()
            graph = rustworkx.PyDiGraph.read_edge_list(
    ...
    def test_labels_digraph_target_existing(self):
        with tempfile.NamedTemporaryFile("wt") as fd:
            fd.write("a|b|0// test a comment\n")
            fd.write("b|c|1\n")
            fd.write("a|c\n")
...
"""
    ...
    def test_invalid_return_type_weight_fn(self):
        ...
        graph = rustworkx.directed_gnm_random_graph(5, 4)
