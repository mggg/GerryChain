...
def callback():
    ...
    for key, value in visOptions.items():
        if vchoices[key].get():
...
def add_to_evallist():
    ...
    allfuncs = [x + ',' + ','.join(y for y, z \
            in columnNameChoices[x].items() if z.get()==True) \
...
def select_vis_type():
    global vchoices
    if vchoices["histogram"].get() == True:
    ...
    if vchoices["save_history_of_changes"].get() == True:
